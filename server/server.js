...
for (var i = 0; i < config.server.staticUrls.length; i++) {
    var staticUrl = config.server.staticUrls[i];
    app.use(staticUrl, express.compress());
    app.use(staticUrl, express.static(config.server.distFolder + staticUrl));
...
// TODO move this logic to auth module
